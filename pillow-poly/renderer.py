...
# Non-dunder introspection attributes. The underscore filter misses these
# because they don't start with "_", yet they reach live frames/globals:
#   (gen).gi_frame.f_back.f_globals['os']  -> the renderer's real os module.
# Generators (gi_*), coroutines (cr_*), async gens (ag_*), frames (f_*) and
# tracebacks (tb_*) all expose the frame/globals chain. format/format_map allow
# attribute-walking via string templates (read-only, but closes the last leak).
_DENIED_ATTR_PREFIXES = ("gi_", "cr_", "ag_", "f_", "tb_")
_DENIED_ATTRS = frozenset({
    "format", "format_map",                       # str template attribute walk
    "func_globals", "func_code", "func_builtins", # py2-style function internals
    "with_traceback",
})
...
def validate_scene(code):
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Attribute):
            a = node.attr
            if a.startswith("_"):
                return f"access to attribute '{a}' is not allowed"
            if a in _DENIED_ATTRS or a.startswith(_DENIED_ATTR_PREFIXES):
                return f"access to attribute '{a}' is not allowed"
